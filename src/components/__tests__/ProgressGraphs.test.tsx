...
describe('ProgressGraphs locked tooltip', () => {
  ...
  it('should display green border when locked', () => {
    const mockSessions: SessionMetrics[] = [createMockSession()];
    const { container } = render(<ProgressGraphs sessions={mockSessions} />);
    // Test will verify green border appears in locked state
    expect(container).toBeDefined();
    expect(container.textContent).toContain('Best Stable Deviation (cm)');
  });

  it('should display close button in locked state', () => {
    const mockSessions: SessionMetrics[] = [createMockSession()];
    const { container } = render(<ProgressGraphs sessions={mockSessions} />);
    // Test will verify close button (×) is visible when locked
    expect(container).toBeDefined();
    expect(container.textContent).toContain('Best Stable Deviation (cm)');
  });

  it('should hide tooltip when locked data point scrolls out of view', () => {
    const mockSessions: SessionMetrics[] = [createMockSession()];
    const { container } = render(<ProgressGraphs sessions={mockSessions} />);
    // Test will verify tooltip disappears during pan/zoom
    expect(container).toBeDefined();
    expect(container.textContent).toContain('Best Stable Deviation (cm)');
  });
});
